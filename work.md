...
> 每条都带**证据**和**该改哪个文件**。做完一条就在这行前面加 ✅。

**P0 — 直接影响项目可信度**

| # | 问题 | 证据 | 改哪 |
|---|---|---|---|
| 1 | **`root_causes` 把"未观测"当成缺口排序** | 作答稀疏时图遍历 Top-1 **0.4933**,反而低于「闭包内随机排序」的 **0.6267** | [coach/knowledge/queries.py](coach/knowledge/queries.py) `detect_gaps` 要区分「未观测」与「已观测且弱」;`profile/store.py` 需要能表达"没有记录"而不是返回 0.1 |
| 2 | **计划推荐"没有题目的知识点"** | 计划 360 步里 **300 步(83%)** 白费;修掉后提升从 -89.7% → -1.2% | [coach/workflow/query.py](coach/workflow/query.py) `plan_view` 过滤 or 补上出题能力(见 #5) |
...
| 4 | **计划没跑赢随机** | 相对随机 **-1.19%** | 先修 #2,再重跑 [run_eval.py](coach/evaluation/run_eval.py);若仍无增益,如实写进 Limitations |
...
| # | 问题 | 说明 |
|---|---|---|
| 8 | 四类关系里三类是死的 | `RELATED` / `EXTENDS` / `CONTRASTS` 存进图了,但**推理只用 `PREREQUISITE`**。可以做对比辨析题、下一步深化 |
