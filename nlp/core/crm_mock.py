...
【现状 · 这里已是死代码】Agent 已全部切到 `core/ecommerce_crm.py`（真实 SQLite），
本文件的 `MockCRM` 在整个仓库里**没有任何 import 或实例化**，不参与运行 ——
保留它只为对照"改造前长什么样"。初学扫一眼类名与方法签名即可，不必逐行研究。

Mock（模拟对象）= 用假的替身顶掉真实依赖：好处是不依赖外部服务、测试可控；
代价是数据永远那几条，写进去什么业务库都不知道。这也正是它被替换掉的原因。
"""
