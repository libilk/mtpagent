...
- **第一段（不调 LLM，秒级）** 数据层与纯逻辑：外键约束、退货三重校验、
  防幻觉兜底、知识图谱推导、知识库白名单、上游结果交接、
  长期记忆（规则与隔离 / 叙事化注入 / 治理规则）
...
这个项目是从一个**通用多 Agent RAG 系统**改造成电商售后助手的，过程完整记录在四份文档里：

| 文档 | 内容 |
|---|---|
| [设计文档.md](设计文档.md) | **入口 · 稳态图纸** —— 结构 / 契约 / 可迁移的 agent 设计结论（现在时，非时间线） |
| [study.md](study.md) | **附录 · 路线与台账** —— 9 个阶段（0~8）的计划、里程碑、进度，以及问题台账（#1~#40） |
| [idea.md](idea.md) | **附录 · 学习笔记** —— 每一处改动「怎么想的 / 为什么这么改 / 人类该从中学到什么」 |
| [术语表.md](术语表.md) | **附录 · 英文术语口径** —— 同一个英文词全项目统一译名，读代码卡住时按词查 |
