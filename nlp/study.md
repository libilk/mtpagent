...
**🔴 一个仍未解决的问题：** 图片那次查询，服务端正常完成（Evaluator 评分 0.87），
但浏览器端没有渲染出答复。**根因未查明** —— 现在 DashScope 账户欠费，无法复现。
（排查过程中我曾误判为「DAG 的 task_2 从未执行」，打桩后证明是错的，
调度完全正常。教训记在 [idea.md](idea.md) 阶段 5 §4.7。）

**→ 但已把这类故障的隐患修掉**（§4.8）：静态排查发现前端有"静默失败"的组合 ——
`removeElement` 在渲染之前、异常只 `console.warn` 不上界面、`appendBotMessage`
假定入参是字符串。合起来正好产生"进度条消失 + 无答复 + 无报错"。
已修三处（调换顺序 / 异常上界面 / 入参兜底），并用合成事件验证通过。
**查不到根因时，至少要让故障可见、可诊断。**
