...
>
> **配套文档**：（2026-09-23 起 [设计文档.md](设计文档.md) 为**唯一入口**，本文件降级为附录）
> [设计文档.md](设计文档.md) —— 稳态图纸：结构 / 契约 / 可迁移结论（现在时，非时间线）。
> [idea.md](idea.md) —— 记录每一处改动「怎么想的 / 为什么这么改 / 人类该从中学到什么」。
> [术语表.md](术语表.md) —— 英文术语的统一译名与一句话解释，读代码卡住时按词查。
> 设计文档管**现状**，本文件管**路线和进度**（含**问题台账**，引用编号形如 #37），
> idea.md 管**理解**，术语表管**口径**，四者同步更新。
...
> **⚠️ 对外的措辞提醒（写简历/答辩时）—— ✅ 2026-09-23 已更正**
>
> 本条原先写的是「不能说"上游结果自动传递给下游""支持链式协作""任务间参数自动对齐"——
> 那三样都没接线」。**那是 #34 修复【之前】的状态。#34 已修，第一条现在成立了**，
> 所以措辞边界**移动了，不是消失了**：
>
> | 现在可以说 | 仍然不能说 |
> |---|---|
> | ✅ "上游任务的**结果原文**会注入下游任务的 query"（`_MAX_UPSTREAM_CHARS = 1500` 截断） | ❌ "**字段级**参数对齐 / 参数自动映射"（`parameter_mapping` 的字段级语义未实现） |
> | ✅ "按依赖关系分波次调度执行" | ❌ "参数校验不通过自动重跑上游"（**#37**，整条链路不执行） |
>
> **被追问时的正确答法：** "上游任务的**结果原文**会拼进下游任务的 query 下发（1500 字截断），
> 所以任务间数据传递是通的。但**传的是结果原文，不是命名字段** —— `parameter_mapping` 声明的
> 字段级映射没有实现，配套的校验与重试链路（#37）当前也不执行。"
>
...
> **【修复状态 2026-09-23】** 三处里可低风险处理的部分**已改**（其余留给决策）：
>
> | 处 | 处理 |
> |---|---|
> | ① `result` 的死防御 | ✅ **已删**（aggregator 那处 `isinstance(answer, dict)`） |
> | ② `options` 假契约 | ✅ **已修** —— 前端改成按 `options` 渲染按钮，不再写死三个 |
> | ③ `audience` 死链 | ⬜ **未动** —— 涉及"要不要恢复文档角色过滤"的产品决策，留给后续 |
> | 附带 | ✅ `context["messages"]` 那条**死转发已删**（#39 的延伸） |
>
> **验证**：回归测试 **20/20 通过**（耗时 148.9s）；前端弹窗已在浏览器实测
> —— 用真实 mock 事件跑了 4 个用例（含"无 options 回退"与"未知值被白名单挡掉"）。
>
> **顺带勘误**：回归用例**实际已有 20 条**，不是文档里写的 15/16。
> 那两处数字**按项目惯例不修**（是对当时状态的忠实记录，见 §4 阶段 6 的说明），
> 此处只记下当前真实值。
