...
| 2026-09-16 | **study.md 新增「核心逻辑(五条)与自测」**。起因:"逐文件读完了,但掌握不了核心逻辑"。诊断是**目录树横切、逻辑纵切** —— 阅读顺序只防得住"不知道全貌",防不住"不知道**为什么**要这样"。新增三块:① **五条核心逻辑表**(洞见 → 衍生设计 → **去掉会怎样**);② **白纸画链路填空自测**(①~⑧ + 对照答案,第 ④ 空最容易答反 —— "先记后做"是把**丢失降级为重复**,不是"避免重复执行");③ **五个破坏性实验**(改一处 → 跑测试 → 看哪些红 → 改回来,含 `transaction()` 打回空操作、`invoke(None, config)` 改传 state、去掉 `sorted(kps)`、`detect_gaps` 去两态、api 里 import knowledge)。自测的设计特性:八个空**不均匀分布** —— ③④⑤⑦⑧ 全指向核心逻辑第 4 条(可靠性),⑥ 指向第 2 条 |
