...
>
> **读完之后读什么:** 见下方 **「核心逻辑(五条)与自测」** —— 阅读顺序防的是"不知道全貌",
> 那一节防的是"不知道**为什么**要这样"(靠白纸画链路填空 + 破坏性实验)。
>
> **怎么用:** 每个文件都按同一套格式写 ——
...
> **这一节解决什么:** 上面的阅读顺序解决"先读什么",这一节解决"**读完了,怎么知道真懂了**"。
> 逐文件读只防得住"不知道全貌",防不住"不知道**为什么**要这样" ——
> 而后者才是被追问时露馅的地方。所以每个设计决策都要能回答**"去掉它会怎样"**。

### 五条核心逻辑

**目录树是横切的,逻辑是纵切的。** 顺着目录读,读到的是一堆独立抽屉;
真正把项目串起来的是下面五条"因为 X,所以必须 Y":

| # | 核心洞见 | 由它衍生的设计 | 去掉会怎样 |
|---|---|---|---|
| 1 | **根因不在当前知识点,在前置** | 图 + 反向遍历:递归 CTE 求闭包、按深度排序 | 退化成"推荐相似题",与扁平检索无异 |
| 2 | **掌握度低的点有两种,数值一样意思不一样**(没观测 ≠ 确实弱) | 排序加"证据态"维度 | 大量"从没考过的浅层点"挤掉真根因(实测稀疏条件 **−17.3 个点**) |
| 3 | **LLM 会瞎说,它的输出不能当事实** | governance:`observation → proposal → aggregator` | 脏数据直写图 → 污染前置闭包,根因定位**原理性失效** |
| 4 | **进程会在任何一步被杀** | 不丢:journal 先记后做 + recovery;不重:双闸幂等;贵的步不重跑:checkpointer | 半途被杀 → 更新**永久丢失**(实测踩过);或 LLM 被重复调用(花钱且结果不同) |
| 5 | **你说有用,得拿数字来** | 评测 + 三个对照臂(消融 / 闭包内随机 / 扁平) | 会把"候选集只有 4 个点"误当成"图推理强" |

**其余全是配套:** BKT / SM-2 是第 2 条的输入(掌握度从哪来);
FastAPI / Redis / 题库导入 / LeetCode 抓取是第 4 条与工程外壳;
agent 支线是第 1 条的工具化包装。

### 自测:白纸画链路,不许翻文档

填 [①~⑧],答不上就写"不会" —— 那不是输,那是**找到了洞**。猜对的也不算学会。

```
POST /answer
  │
  ▼
[api] 校验 → ①______ → publish(coach:answer) → 202 返回
       ② 这层为什么绝对不能调 LLM?
  │
  ▼
[coord] XREADGROUP 取消息
  │ ③ 先查 ______ 表(作用:______)
  │ ④ 再写 ______(先记后做),为什么必须"先记"?
  ▼
[pipeline] LangGraph 链
  ├─[1] grade
  ├─[2] update_profile   ← ⑤ 为什么 BKT 和 SM-2 必须合成一个节点?
  ├─[3] detect_gap       ← ⑥ 排序用哪三个维度?为什么要"证据态"这一维?
  │      ├─ 有缺口 → [4] explain ← ⑦ 为什么它单独成节点?
  │      └─ 无缺口 → finalize
  └─[5] finalize
  │
  ▼
[coord] ACK → 失败重投 → 超过 ______ 次进 ⑧______
```

### 对照答案

**① 生成 `event_id`(ULID)。**
为什么必须在**最边缘**生成、不能在消费端生成?因为下游整条幂等链都挂在这个 id 上 ——
消费端自己生成的话,同一条消息投两次就是两个不同 id,幂等表认为它们是两件事,
**幂等直接失效**。→ *幂等的种子必须在源头埋。*

**② 不止是速度。** LLM 慢(几百 ms ~ 几秒)、贵(花钱)、非幂等(采样不确定)、会失败。
它在请求路径上 = **入口的延迟与可用性被 LLM 绑架**。
所以拆开:入口只校验 + 入队,LLM 交 worker 异步做、结果写缓存,`/gap` 只读缓存。

**③ `processed_events` 表。**
为什么需要它?Redis Stream 是 **at-least-once** —— 消费了但 ACK 前崩溃，
消息**会被重新投递**。所以业务必须自己挡第二次。

**④ journal。★ 这条最容易答反,因果链是:**

- **先做后记**:执行完、还没记,进程被杀 → 没有任何痕迹 → recovery 扫不到 →
  这次更新**永远丢失**,而且是静默的
- **先记后做**:记了、没做完,进程被杀 → recovery 扫到 pending → 补做 → 最坏是**做了两次**

> **先记后做把「丢失」降级为「重复」,而重复可以用幂等挡掉。**
> **丢失不可修复,重复可以过滤。**

"避免重复执行"是**幂等表**的职责,不是 journal 的职责。这是两件事。

**⑤ 因为多步写必须原子。** 事务解决的是**半成品**状态:
BKT 写了、SM-2 没写 → 进程被杀 → 重放时看到答题记录已存在 → **跳过** →
SM-2 永远停在旧值。拆成两个节点 = 让这个状态变成可能。

**⑥ 三个维度:证据态 → 深度 → 掌握度。**
为什么"证据态"必须排第一?闭包里掌握度低的点有两种,数值一样、含义完全不同:
没观测过(掌握度只是初始值 `P_init=0.1`,看着很低)vs 观测了确实弱。
只按 `(深度, 掌握度)` 排 → 最浅的、从没考过的点全部排前面,真根因被挤掉。

**⑦ 因为它是链里唯一"贵且非幂等"的一步。**
单独成节点 → 它的产出先落检查点 → 后面崩了不用重调 LLM。
否则从头重放:钱花了第二遍,而且因为采样不确定,**结果还和上次不一样**。
`test_pipeline.py` 用 **LLM 调用计数**断言这件事。

**⑧ 超过 3 次 → 死信队列(DLQ)+ 告警。**

> **这个自测的设计特性:** 八个空**不是均匀分布**的 ——
> ③④⑤⑦⑧ 全指向核心逻辑第 4 条(可靠性),⑥ 指向第 2 条。
> 如果多数空答不上来,缺的不是"这个项目",而是**可靠性设计与度量严谨性**这两条通用能力。

### 破坏性实验(比读代码牢十倍)

读是输入,**核心逻辑只能靠"制造一次失败"长出来**。
每个实验 15 分钟:改 → 跑 `pytest tests/coach/ -q` → 看哪些红 → **改回来** → 用自己的话写下"这块代码存在的理由"。

| # | 改什么 | 预期炸哪 | 学到什么 |
|---|---|---|---|
| 1 | `ProfileStore.transaction()` 改成空操作 | `test_worker.py` 的 3 项原子性测试 | 事务是被一次**真实数据丢失**逼出来的 |
| 2 | `pipeline.run()` 的 `invoke(None, config)` 改成传 state | `test_pipeline.py::test_crash_after_explain_does_not_recall_llm` | checkpointer 存的不是"结果",是"下一步该跑谁" |
| 3 | 去掉 `run_eval.py` 里的 `sorted(kps)` | 跨进程跑两次,数字不同 | `PYTHONHASHSEED` 怎么毁掉评测 |
| 4 | `detect_gaps` 排序里去掉两态区分 | 消融臂数字掉 | "未观测" ≠ "已确认薄弱" |
| 5 | 在 `coach/api/` 里 import `knowledge` | `test_api.py::test_api_source_has_no_forbidden_imports` | 分层硬约束是**被测试锁死的**,不是文档口号 |

**顺带一提:** 实验 1 这个手法项目里已经有人用过 —— work.md §12 记着
"把 `transaction()` 打回空操作后其中 3 项失败(已验证测试有效)"。
**用测试当探针去理解实现**,是这里最省时间的一招。
