...
22 个知识点 / 19 道题)。完整数字在 [RESULTS.md](coach/evaluation/results/RESULTS.md)。

### ★ 表 4:根因定位 —— 图遍历 vs 扁平召回

| 条件 | 臂 | Top-1 | Recall@k |
|---|---|---|---|
| 作答覆盖充分 | **图遍历(本方案)** | **0.5467** | 0.8400 |
| | 图遍历 · 消融(不区分"有没有证据") | 0.5467 | 0.8400 |
| | 闭包内随机排序 | 0.5067 | 0.9600 |
| | 扁平召回(文本相似度) | 0.0800 | 0.1867 |
| 作答稀疏 | **图遍历(本方案)** | **0.6800** | 0.8933 |
| | 图遍历 · 消融(不区分"有没有证据") | 0.5200 | 0.8933 |
| | 闭包内随机排序 | 0.6267 | 0.9600 |
| | 扁平召回(文本相似度) | 0.1333 | 0.3067 |
...
| 表 | 指标 | 结果 | |
|---|---|---|---|
| 1 掌握度预测 | AUC / Brier | **0.5336** / **0.2741** | ❌ 比"恒定预测基准率"(Brier 0.2498)还差 |
| 2 遗忘预测 | ECE(隔 30 天) | 0.2406 → **0.3450** | ✅ 印证 BKT 不建模遗忘:预测 0.42,实际 0.08 |
| 3 规划增益 | 相对随机 | **+0.6%** | ⚠️ 白费步数 82%→**0%**,但领先幅度太小,不足以声称有效 |
...
消融实验显示,区分这两态在作答稀疏时带来 **+17.3 个点**的 Top-1 提升
(作答充分时本次为 **+0.0 点** —— 证据足够时两态区分不改变结论,符合预期)。
...
3. **BKT 的掌握度预测基本不可用**(表 1)。AUC 0.5336,Brier 比恒定基准率还差。
   原因:`P_init = 0.1` 系统性低估,且连续答对约 10 次后 `p_known` 饱和到 1.0。
   这是照抄公式的必然结果,不是实现 bug。
   **本项目因此把它降级为「只用于排序,不用于预测」**,并在 `bkt.py` 里写明。
4. **规划的价值没有被证实**(表 3,仅 +0.6%)。诊断出的原因是**计划作用域太窄**:
   `plan_view` 只覆盖目标的前置闭包(本图里 `algo.dp` 的闭包只有 4 个点),
   前置补完、目标也达标之后,计划就真的没事可做了 —— 实测 24 步里约一半返回空计划,
   此后比的是"随机练"对"什么都不练",已经不是规划的评估。
   要让规划真正有用,得让它在目标达成后能提出**下一个目标**,当前设计里没有这个概念。
5. **题库覆盖是根因定位的必要条件。** 一个知识点没有题,就永远产生不了作答证据,
   也就永远无法被定位为根因。本项目已把题库扩到覆盖全部 22 个知识点 ——
   但这条前提本身值得记住:换更小的题库,根因定位的数字会直接掉。
6. **规模小。** 22 个知识点、19 道题、30 个模拟学生。同一种子下结果可复现
   (评测里的集合迭代一律排序后再遍历 —— 否则 `PYTHONHASHSEED` 会让数字逐进程漂移),
   但换种子结论方向可能变。
