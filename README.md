...
| 条件 | 臂 | Top-1 | Recall@k |
|---|---|---|---|
| 作答覆盖充分 | **图遍历(本方案)** | **0.6000** | 0.8933 |
| | 图遍历 · 消融(不区分"有没有证据") | 0.6000 | 0.8933 |
| | 闭包内随机排序 | 0.5067 | 0.9867 |
| | 扁平召回(文本相似度) | 0.0800 | 0.2267 |
| 作答稀疏 | **图遍历(本方案)** | **0.6667** | 0.8133 |
| | 图遍历 · 消融(不区分"有没有证据") | 0.4933 | 0.8133 |
| | 闭包内随机排序 | 0.6267 | 0.9600 |
| | 扁平召回(文本相似度) | 0.1200 | 0.3067 |
...
| 3 规划增益 | 相对随机 | **+1.2%** | ⚠️ 白费步数 82%→**0%**,但领先幅度太小,不足以声称有效 |
...
4. **规划的价值没有被证实**(表 3,仅 +1.2%)。诊断出的原因是**计划作用域太窄**:
...
6. **规模小。** 22 个知识点、19 道题、30 个模拟学生,单个数字有随机波动。
